--//=========================================================
--// 크랙본 by R0W | Soft sky tint via Atmosphere
--// F1 UI, F2 purge. Drag via top bar. All toggles OFF.
--//=========================================================
...
--//=========================================================
--// SKY TINT — soft, low-density Atmosphere
--//=========================================================
local SkyState = {
    savedSky   = nil,
    savedAtm   = nil,
...
local function killDynamic()
    ...
    SkyState.appliedSky = nil
    SkyState.appliedAtm = nil
end

local function restoreOriginal()
    killDynamic()
    if SkyState.savedSky then
        local c = SkyState.savedSky:Clone()
        c.Parent = Lighting
    end
    if SkyState.savedAtm then
        local c = SkyState.savedAtm:Clone()
        c.Parent = Lighting
    end
...
-- SOFT tint: low density, muted colors, no fog on terrain
local function applyAtmosphereTint(color, opts)
    opts = opts or {}
    killDynamic()

    local atm = Instance.new("Atmosphere")
    atm.Name        = "crackbonAtm"
    atm.Color       = color
    atm.Decay       = opts.decay or Color3.fromRGB(80, 90, 105)
    atm.Density     = opts.density or 0.22   -- low = subtle
...
local function applyRealSky(images)
    ...
    sky.SkyboxBk      = images.Bk
    sky.SkyboxDn      = images.Dn
    sky.SkyboxFt      = images.Ft
    sky.SkyboxLf      = images.Lf
    sky.SkyboxRt      = images.Rt
    sky.SkyboxUp      = images.Up
...
local function applySkybox(name)
    ...
    elseif name == "Space" then
        applyRealSky(SPACE_SKY)

    elseif name == "Red" then
        applyAtmosphereTint(Color3.fromRGB(230, 150, 150), {
            density=0.20, haze=1.2, glare=0.15,
            ambient=Color3.fromRGB(120,100,100),
            outdoor=Color3.fromRGB(180,140,140),
        })
    elseif name == "Orange" then
        applyAtmosphereTint(Color3.fromRGB(240, 190, 140), {
            density=0.20, haze=1.2, glare=0.20,
            ambient=Color3.fromRGB(130,110,90),
            outdoor=Color3.fromRGB(200,170,140),
        })
    elseif name == "Yellow" then
        applyAtmosphereTint(Color3.fromRGB(240, 230, 170), {
            density=0.20, haze=1.2, glare=0.25,
            ambient=Color3.fromRGB(140,135,105),
            outdoor=Color3.fromRGB(210,200,170),
        })
    elseif name == "Green" then
        applyAtmosphereTint(Color3.fromRGB(160, 220, 175), {
            density=0.20, haze=1.2, glare=0.15,
            ambient=Color3.fromRGB(105,130,110),
            outdoor=Color3.fromRGB(160,200,170),
        })
    elseif name == "Blue" then
        applyAtmosphereTint(Color3.fromRGB(160, 190, 235), {
            density=0.20, haze=1.2, glare=0.15,
            ambient=Color3.fromRGB(105,120,150),
            outdoor=Color3.fromRGB(160,185,220),
        })
    elseif name == "Purple" then
        applyAtmosphereTint(Color3.fromRGB(205, 175, 235), {
            density=0.20, haze=1.2, glare=0.15,
            ambient=Color3.fromRGB(125,110,150),
            outdoor=Color3.fromRGB(185,165,215),
        })
    elseif name == "Pink" then
        applyAtmosphereTint(Color3.fromRGB(240, 190, 215), {
            density=0.20, haze=1.2, glare=0.15,
            ambient=Color3.fromRGB(140,115,125),
            outdoor=Color3.fromRGB(210,180,195),
        })

    elseif name == "Night" then
        applyAtmosphereTint(Color3.fromRGB(60, 75, 120), {
            density=0.25, haze=0.8, glare=0,
            clockTime=0,
            ambient=Color3.fromRGB(40,50,80),
            outdoor=Color3.fromRGB(55,70,100),
            brightness=1.2,
        })
    elseif name == "Sunset" then
        applyAtmosphereTint(Color3.fromRGB(240, 175, 140), {
            density=0.22, haze=1.5, glare=0.3,
            clockTime=17.5,
            ambient=Color3.fromRGB(120,95,85),
            outdoor=Color3.fromRGB(200,165,140),
        })
    elseif name == "Dawn" then
        applyAtmosphereTint(Color3.fromRGB(235, 190, 210), {
            density=0.22, haze=1.3, glare=0.25,
            clockTime=6,
            ambient=Color3.fromRGB(120,105,115),
            outdoor=Color3.fromRGB(200,180,190),
        })
    end
end
...
local function makeToggle(parentCol, label, default, callback)
    ...
    api.set(default)
    box.MouseButton1Click:Connect(function() api.set(box.BackgroundColor3 ~= Colors.On) end)
    return api
end

local function makeSlider(parentCol, label, min, max, default, decimals, callback)
    ...
    local function update(input)
        ...
        fill.Size = UDim2.new(rel,0,1,0); val.Text = tostring(v)
        if callback then callback(v) end
    end
...
local function makeDropdown(parentCol, label, options, default, callback)
    ...
    for i, opt in ipairs(options) do
        ...
        o.MouseButton1Click:Connect(function() btn.Text=opt; list.Visible=false; if callback then callback(opt) end end)
    end
...
--// KEYBIND PICKER
local function makeKeybind(parentCol, label, defaultBind, callback)
    ...
    return { get = function() return currentBind end }
end
...
do
    ...
    makeToggle(L, "Enabled", Config.Aimbot.Enabled, function(v) Config.Aimbot.Enabled = v end)
    makeDropdown(L, "Target Part", {"Head","UpperTorso","LowerTorso","Nearest"}, Config.Aimbot.TargetPart, function(v) Config.Aimbot.TargetPart = v end)
    makeSlider(L, "Smoothness", 0.01, 1, Config.Aimbot.Smoothness, 2, function(v) Config.Aimbot.Smoothness = v end)
    makeSlider(L, "Prediction", 0, 1, Config.Aimbot.Prediction, 2, function(v) Config.Aimbot.Prediction = v end)
    makeSlider(L, "Max Distance", 0, 3000, Config.Aimbot.MaxDistance, 0, function(v) Config.Aimbot.MaxDistance = v end)
    makeToggle(L, "Scriptable Cam", Config.Aimbot.ScriptableCamera, function(v) Config.Aimbot.ScriptableCamera = v end)
    makeSection(R, "Aimbot FOV")
    makeSlider(R, "FOV", 0, 500, Config.Aimbot.FOV, 0, function(v) Config.Aimbot.FOV = v end)
    makeToggle(R, "Draw FOV", Config.Aimbot.DrawFOV, function(v) Config.Aimbot.DrawFOV = v end)
    makeToggle(R, "Snapline", Config.Aimbot.Snapline, function(v) Config.Aimbot.Snapline = v end)
    makeSection(R, "Aimbot Checks")
    makeToggle(R, "Team Check", Config.Aimbot.TeamCheck, function(v) Config.Aimbot.TeamCheck = v end)
    makeToggle(R, "Visibility Check", Config.Aimbot.VisCheck, function(v) Config.Aimbot.VisCheck = v end)
    makeToggle(R, "Wall Check", Config.Aimbot.WallCheck, function(v) Config.Aimbot.WallCheck = v end)
end
...
do
    ...
    makeToggle(L, "Enabled", Config.Triggerbot.Enabled, function(v) Config.Triggerbot.Enabled = v end)
    makeToggle(L, "Use Key Hold", Config.Triggerbot.UseKey, function(v) Config.Triggerbot.UseKey = v end)
    makeKeybind(L, "Keybind", Config.Triggerbot.Bind, function(b) Config.Triggerbot.Bind = b end)
    makeSlider(L, "Key FOV", 0, 500, Config.Triggerbot.KeyFOV, 0, function(v) Config.Triggerbot.KeyFOV = v end)
    makeSlider(L, "Key Smoothness", 0.05, 1, Config.Triggerbot.KeyAimSmoothness, 2, function(v) Config.Triggerbot.KeyAimSmoothness = v end)
    makeSlider(L, "Key Prediction", 0, 1, Config.Triggerbot.KeyPrediction, 2, function(v) Config.Triggerbot.KeyPrediction = v end)
    makeSection(L, "Auto Mode (Use Key OFF)")
    makeDropdown(L, "Mode", {"FOV", "Crosshair"}, Config.Triggerbot.Mode, function(v) Config.Triggerbot.Mode = v end)
    makeDropdown(L, "Target Part", {"Head","UpperTorso","LowerTorso","Nearest"}, Config.Triggerbot.TargetPart, function(v) Config.Triggerbot.TargetPart = v end)
    makeSlider(L, "FOV", 0, 500, Config.Triggerbot.FOV, 0, function(v) Config.Triggerbot.FOV = v end)
    makeToggle(L, "Draw FOV", Config.Triggerbot.DrawFOV, function(v) Config.Triggerbot.DrawFOV = v end)
    makeSection(R, "Shared")
    makeSlider(R, "Delay (ms)", 0, 500, Config.Triggerbot.Delay, 0, function(v) Config.Triggerbot.Delay = v end)
    makeSlider(R, "Click Duration (ms)", 0, 300, Config.Triggerbot.ClickDuration, 0, function(v) Config.Triggerbot.ClickDuration = v end)
    makeSlider(R, "Hitchance", 0, 100, Config.Triggerbot.Hitchance, 0, function(v) Config.Triggerbot.Hitchance = v end)
    makeSlider(R, "Max Distance", 0, 2000, Config.Triggerbot.MaxDistance, 0, function(v) Config.Triggerbot.MaxDistance = v end)
    makeToggle(R, "Burst Mode", Config.Triggerbot.Burst, function(v) Config.Triggerbot.Burst = v end)
    makeSlider(R, "Burst Count", 1, 10, Config.Triggerbot.BurstCount, 0, function(v) Config.Triggerbot.BurstCount = v end)
    makeSection(R, "Checks")
    makeToggle(R, "Team Check", Config.Triggerbot.TeamCheck, function(v) Config.Triggerbot.TeamCheck = v end)
    makeToggle(R, "Visibility Check", Config.Triggerbot.VisCheck, function(v) Config.Triggerbot.VisCheck = v end)
end
...
do
    ...
    makeToggle(L, "Enabled", Config.Visuals.Enabled, function(v) Config.Visuals.Enabled = v end)
    makeToggle(L, "Team Check", Config.Visuals.TeamCheck, function(v) Config.Visuals.TeamCheck = v end)
    makeToggle(L, "Visibility Check", Config.Visuals.VisCheck, function(v) Config.Visuals.VisCheck = v end)
    makeSlider(L, "Max Distance", 0, 5000, Config.Visuals.MaxDistance, 0, function(v) Config.Visuals.MaxDistance = v end)
    makeSection(L, "Box")
    makeToggle(L, "Box", Config.Visuals.Box, function(v) Config.Visuals.Box = v end)
    makeToggle(L, "Box Filled", Config.Visuals.BoxFilled, function(v) Config.Visuals.BoxFilled = v end)
    makeSlider(L, "Box Padding", 0, 2, Config.Visuals.BoxPadding, 2, function(v) Config.Visuals.BoxPadding = v end)
    makeSection(L, "Chams")
    makeToggle(L, "Chams", Config.Visuals.Chams, function(v) Config.Visuals.Chams = v end)
    makeToggle(L, "Through Walls", Config.Visuals.ChamsThroughWalls, function(v) Config.Visuals.ChamsThroughWalls = v end)
    makeSlider(L, "Chams Transparency", 0, 1, Config.Visuals.ChamsTransparency, 2, function(v) Config.Visuals.ChamsTransparency = v end)
    makeSection(R, "Info")
    makeToggle(R, "Name", Config.Visuals.Name, function(v) Config.Visuals.Name = v end)
    makeToggle(R, "Distance", Config.Visuals.Distance, function(v) Config.Visuals.Distance = v end)
    makeToggle(R, "Health", Config.Visuals.Health, function(v) Config.Visuals.Health = v end)
    makeToggle(R, "Head Dot", Config.Visuals.HeadDot, function(v) Config.Visuals.HeadDot = v end)
    makeToggle(R, "Tracer", Config.Visuals.Tracer, function(v) Config.Visuals.Tracer = v end)
    makeToggle(R, "Skeleton", Config.Visuals.Skeleton, function(v) Config.Visuals.Skeleton = v end)
    makeToggle(R, "Team Color", Config.Visuals.TeamColor, function(v) Config.Visuals.TeamColor = v end)
end
...
do
    ...
    makeSection(L, "하늘 색")
    makeDropdown(L, "Sky Preset",
        {"Default","Space","Red","Orange","Yellow","Green","Blue","Purple","Pink","Night","Sunset","Dawn"},
        "Default",
        function(v) applySkybox(v) end
    )

    makeSection(L, "조명")
    makeToggle(L, "Fullbright", false, function(v)
        if v then
    ...
    makeSection(R, "Config")
    makeSection(R, "Keybinds")
end
